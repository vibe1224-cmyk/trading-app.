...
with tab_hot:
    ...
    c1, c2 = st.columns(2)
    with c1:
        speed = st.radio("模式", ["⚡ 快速（每類股1檔，約12檔）",
                                  "🐢 完整（每類股3檔，約36檔）"],
                         index=0)
    with c2:
    ...
    win = {"近5日（短線）": 5, "近20日（一個月）": 20}[hot_days]
 
    if speed.startswith("⚡"):
        SECTORS = {k: v[:1] for k, v in SECTORS_FULL.items()}
        st.caption("快速模式：約12次資料下載，通常20-40秒完成。")
    else:
        SECTORS = SECTORS_FULL
        st.caption("完整模式：約36次資料下載，可能要1-2分鐘，"
                   "在免費環境有時會逾時失敗。建議先用快速模式。")
...
with tab2:
    ...
    c1, c2 = st.columns(2)
    with c1:
        bsym = st.text_input("股票代號（逗號分開，最多3檔）",
                             "2330.TW", key="bs")
    with c2:
        byears = st.radio("回測長度", ["5年（快）", "10年（慢）"],
                          horizontal=True, index=0)
    ...
    bperiod = "5y" if byears.startswith("5") else "10y"
    st.caption("💡 建議先用單一檔、5年跑一次，確認能動之後再加檔數或拉長年限。"
               "台股一買一賣約 0.585%，加滑價，單趟抓 0.35% 比較保守。")
 
    if st.button("開始回測", use_container_width=True, type="primary", key="bb"):
        syms = [fix_symbol(x) for x in bsym.split(",") if x.strip()][:3]
        rows = []
        bar = st.progress(0.0)
